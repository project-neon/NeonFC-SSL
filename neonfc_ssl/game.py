...
class Game:
    ...
    def start(self):
        ...
        for layer in self.layers.values():
            if layer.previous_layer == "":
                continue
            previous = self.layers.get(layer.previous_layer, None)
            if previous is None:
                raise Exception("No previous layer")
            layer.bind_input_pipe(previous.output_pipe)
    ...
    def new_layer(self, layer: type['Layer']):
        ...
        self.layers[layer_obj.name] = layer_obj
