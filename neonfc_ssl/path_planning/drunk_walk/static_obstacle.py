...
# from typing import Tuple  # removed because it was unused
import numpy as np


# Static obstacle: will be considered a rectangle, so its vertices will be
# stored. Collision checking will be simpler
@dataclass
class StaticObstacle(Obstacle):
    ...
    def check_for_collision(
        ...
        # if (
        #    self.start[0] <= point[0] <= self.end[0]
        #    and self.start[1] <= point[1] <= self.end[1]
        # ):
        #     print(f"{self.start=}")
        #     print(f"{point=}")
        #     print(f"{self.end=}")
