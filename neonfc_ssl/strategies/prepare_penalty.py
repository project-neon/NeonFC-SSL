...
class PrepPenalty(BaseStrategy):
    ...
    def position(self):
        ...
        i = 0
        for j in range(1, len(self._match.active_robots)-1):
            penalty_pos[i][0] = field.fieldLength/2 + 0.2
            penalty_pos[i][1] = (j * (5/6)) + (0.2 * (j - 1)) + 0.1
            i += 1
    
        theta = atan2(-self._robot.y + self._match.ball.y, -self._robot.x + self._match.ball.x)

        self.cost_matrix(penalty_pos, self.defensive_positions)
