...
class RRT:
    ...
    def is_collision_free(self, start: Node, end: Node) -> bool:
        ...
        # Original version of the sec function, unused to avoid lambda
        # functions following pep8 recomendations.
        # sec = lambda o: (
        #     (abs(A*o.x + B*o.y + C) / div)
        #     > self.collision_margin
        #     )

        def sec(o):
            return (abs(A*o.x + B*o.y + C) / div) > self.collision_margin
...
class RRTStar(RRT):
    ...
    def find_neighbors(self, node):
        return [
            ...
            if np.linalg.norm([n.x - node.x, n.y - node.y])
            <= self.search_radius
            ]
    ...
    def choose_parent(self, neighbors, nearest, node):
        min_cost = (
            nearest.cost
            + np.linalg.norm([nearest.x - node.x, nearest.y - node.y])
            )
        ...
        for neighbor in neighbors:
            cost = (
                neighbor.cost
                + np.linalg.norm([neighbor.x - node.x, neighbor.y - node.y])
                )
    ...
    def rewire(self, node, neighbors):
        for neighbor in neighbors:
            ...
            if (
                cost < neighbor.cost
                and self.is_collision_free(neighbor, node)
            ):
