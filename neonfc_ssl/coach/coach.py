...
import numpy as np
from neonfc_ssl.coach.coach_data import CoachData, RobotCommand
from neonfc_ssl.core import Layer
from typing import TYPE_CHECKING, Union, Optional
if TYPE_CHECKING:
    from neonfc_ssl.match.match_data import MatchData, TrackedRobot
    from neonfc_ssl.strategies import BaseStrategy
...
class Decision(Layer):
    ...
    def decide(self, data: 'MatchData'):
        ...
        For setting hungarian robots use the self.calculate_hungarian method
        For setting special robots use the self.set_strategy method
    ...
    def set_strategy(self, robot: 'TrackedRobot', strategy: type['BaseStrategy']):
        if robot.id > len(self.__strategies):
            self.log(logging.ERROR, "Trying to set strategy for unknown id {}".format(id))
            return
        if isinstance(self.__strategies[robot.id], strategy):
            return

        self.__strategies[robot.id] = strategy(robot)
    ...
    def calculate_hungarian(self, targets, robots):
        try:
