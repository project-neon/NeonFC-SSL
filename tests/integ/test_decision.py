...
def test_coach_layer(base_config):
    layer_obj, pipe_in, pipe_out, log_q = instance_layer(Decision, base_config)

    ids = [0]

    test_data = tracking_data.MatchData(
    ...
    response:decision_data.DecisionData = pipe_out.recv()

    assert isinstance(response, decision_data.DecisionData), "Response should be a DecisionData instance"

    # assert response.world_model == test_data, "world_model should be a MatchData instance"
